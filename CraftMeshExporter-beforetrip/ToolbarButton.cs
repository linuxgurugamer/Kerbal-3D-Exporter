...
namespace CraftMeshExporter
{
    ...
    //      points at -- see the Reference entries in CraftMeshExporter.csproj).
    ...
    // from actually creating the button (see CraftMeshExporterToolbarButton below), and every mod
    // using ToolbarControl needs to do this with its own unique MODID/MODNAME pair.
    [KSPAddon(KSPAddon.Startup.Instantly, true)]
    public class CraftMeshExporterToolbarRegistration : MonoBehaviour
    {
        public void Start()
        {
            ToolbarControl.RegisterMod(CraftMeshExporterToolbarButton.MODID, CraftMeshExporterToolbarButton.MODNAME);
        }
    }
    ...
    public class CraftMeshExporterToolbarButton : MonoBehaviour
    {
        public const string MODID = "CraftMeshExporterButton";
        public const string MODNAME = "Craft Mesh Exporter";

        // GameDatabase-style paths: relative to GameData, no file extension.
        private const string StockIconPath = "CraftMeshExporter/PluginData/toolbar-icon";
        private const string BlizzyIconPath = "CraftMeshExporter/PluginData/toolbar-icon-24";
        ...
        private void AddButton()
        {
            if (toolbarControl != null)
                return;
            Debug.Log("CraftMeshExporter, Toolbarbutton.AddButton 2");
            ...
            // If the window happens to already be open (e.g. it was opened via the part right-click
            // action before the toolbar was ready), start the button in the matching pressed state.
            //if (CraftPrintExporterWindow.IsWindowOpen())
            //    SetButtonState(true);
        }
        ...
        private static void SetButtonState(bool active)
        {
            ...
            CraftMeshExporterToolbarButton instance = FindObjectOfType<CraftMeshExporterToolbarButton>();
