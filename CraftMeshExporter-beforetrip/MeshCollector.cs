...
namespace CraftMeshExporter
{
    internal static class MeshCollector
    {
            ...
            List<string> diagnostics)
        {
            ...
            if (diagnostics != null)
            {
                diagnostics.Add("CraftMeshExporter mesh diagnostics");
                diagnostics.Add("==================================");
                diagnostics.Add("Lines marked EXPORT are written to the model. Lines marked SKIP are not.");
                diagnostics.Add("Use PartName and Path/Material tokens here in GameData/CraftMeshExporter/shroud-exclusions.txt");
            ...
            foreach (Part part in parts)
            {
                ...
                foreach (MeshFilter mf in filters)
                {
                    ...
                    string reason = GetSkipReason(part, mf.transform, mr, hideEngineShroud, bottomNodeHasNoAttachment, shroudTransformsToSkip, inactiveVariantTransformsToSkip, originallyHiddenVariantTransforms, exclusionRules, disabledRendererKeys, disabledRenderers, disabledRendererTransforms);
                    if (reason != null)
                    {
                    ...
                    AddMeshTriangles(triangles, mf.sharedMesh, mf.transform, finalScale, mr, hideEngineShroud, originOffset);
                ...
                foreach (SkinnedMeshRenderer smr in skins)
                {
                    ...
                    string reason = GetSkipReason(part, smr.transform, smr, hideEngineShroud, bottomNodeHasNoAttachment, shroudTransformsToSkip, inactiveVariantTransformsToSkip, originallyHiddenVariantTransforms, exclusionRules, disabledRendererKeys, disabledRenderers, disabledRendererTransforms);
                    if (reason != null)
                    {
                    ...
                    int before = triangles.Count;
                    AddMeshTriangles(triangles, baked, smr.transform, finalScale, smr, hideEngineShroud, originOffset);
